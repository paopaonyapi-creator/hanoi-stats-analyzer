...
export function LiveMonitor() {
  ...
  // Use ref instead of state to avoid stale closure in the now-based useEffect
  const lastSyncStatusRef = useRef<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    // ─── AUTO-SYNC LOGIC (uses ref — no stale closure) ───
    newStatuses.forEach((s) => {
      const prev = lastSyncStatusRef.current[s.type];
      if (s.status === "CLOSED" && prev !== "CLOSED") {
        lastSyncStatusRef.current[s.type] = "CLOSED";
        handleAutoSync(s.label);
      } else if (s.status !== "CLOSED" && prev === "CLOSED") {
        lastSyncStatusRef.current[s.type] = s.status;
      }
    });
    ...
    <div className="relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {statuses.map((s) => (
          <div
            key={s.type}
            className={`glass-card p-4 flex items-center justify-between border-l-4 ${s.status === "LIVE"
                ? "border-l-[var(--accent-emerald)] bg-[rgba(16,185,129,0.05)]"
                : s.status === "SOON"
                  ? "border-l-[var(--accent-blue)]"
                  : "border-l-[var(--text-muted)] opacity-80"
              }`}
          >
            <div className="flex flex-col">
              <span className="text-xs text-[var(--text-muted)] font-medium mb-1 uppercase tracking-wider">
                {s.label}
              </span>
            ...
            <div className="text-right">
              <span
                className={`text-[10px] px-2 py-0.5 rounded uppercase font-bold ${s.status === "LIVE"
